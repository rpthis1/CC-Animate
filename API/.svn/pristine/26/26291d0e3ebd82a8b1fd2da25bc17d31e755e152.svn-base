...
{
    public class GenericTreeController : ApiController
    {
        // GET api/ipmvpd

        private readonly IGenericTreeProcessor _genericTreeProcessor;
        public GenericTreeController(IGenericTreeProcessor genericTreeProcessor)
        {
            this._genericTreeProcessor = genericTreeProcessor;
        }
        public HttpResponseMessage Get()
        {
            ...
            string result = _genericTreeProcessor.Get(Convert.ToInt32(cluster), Convert.ToInt32(record));
        ...
        {
            string result = _genericTreeProcessor.Get(cluster, record);
            HttpResponseMessage response = Request.CreateResponse(HttpStatusCode.OK);
            response.Content = new StringContent(result.ToString(), Encoding.UTF8, "text/html");
            return response;
        }
