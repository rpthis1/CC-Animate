...
{
    public class TemplateNodeClient : ITemplateNodeClient
    {
        ...
        public TemplateNodeClient(IOrientClient orient,IJsonHelper json, ITemplateLibraryMenuFactory menuFactory)
        {
            _orient = orient;
            _json = json;
        ...
        {
            string json = _json.Serialize(_menuFactory.CreateTabNavAndReportMenu());
           json = HttpUtility.UrlEncode(json);
            var clientObject = (JObject) p;
            var vertex = clientObject.ToObject<BiaVBase>();
            string parameter = String.Format("/TmpTreeCreateTemplate/{0}/{1}/{2}:{3}", vertex.Label, json, cluster, record);
