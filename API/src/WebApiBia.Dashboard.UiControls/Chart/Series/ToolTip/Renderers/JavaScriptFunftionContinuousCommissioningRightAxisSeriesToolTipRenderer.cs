...
{
    public class JavaScriptFunftionContinuousCommissioningRightAxisSeriesToolTipRenderer : JavaScripFunctionAxisRenderer
    {
        public JavaScriptFunftionContinuousCommissioningRightAxisSeriesToolTipRenderer()
        {
        ...
        private void GenerateFunctionBoby()
        {
            ...
            sb.Append("{");
            sb.Append("   this.setHtml( item.field + ':  ' + value + ' ' + options.rightUnits + '  (' + storeItem.get('actualhour') +')' );");
            sb.Append("}");
