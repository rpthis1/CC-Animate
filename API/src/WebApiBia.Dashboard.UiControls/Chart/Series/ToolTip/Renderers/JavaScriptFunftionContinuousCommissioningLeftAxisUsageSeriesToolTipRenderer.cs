...
{
    public class JavaScriptFunftionContinuousCommissioningLeftAxisUsageSeriesToolTipRenderer : JavaScripFunctionAxisRenderer
    {
        public JavaScriptFunftionContinuousCommissioningLeftAxisUsageSeriesToolTipRenderer()
        {
        ...
        private void GenerateFunctionBoby()
        {
            ...
            sb.Append("{");
            sb.Append("   this.setHtml( item.field + ':  ' + value + ' ' + options.leftUnits + '  (' + storeItem.get('actualhour') +')' );");
            sb.Append("}");
            sb.Append("else if( options.mode.toLowerCase() === 'financial')");
            sb.Append("{");
            sb.Append("   this.setHtml(  item.field + ':  $' + value  + '  (' + storeItem.get('cat') +')' );");
            sb.Append("}");
