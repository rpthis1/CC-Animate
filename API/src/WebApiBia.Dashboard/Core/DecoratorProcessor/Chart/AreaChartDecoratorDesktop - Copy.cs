...
namespace WebApiBia.Dashboard.Core.DecoratorProcessor.Chart
{
    public class AreaChartDecoratorDesktop:Decorator
    {
        ...
        protected override void DecorateCategoryAxis(ReportConfig config)
        {
            ...
            if (config.CategoryDataTipLabelField != "")
            {
                JavaScriptFunftionCategoryAxesRenderer renderer = new JavaScriptFunftionCategoryAxesRenderer();
                Chart.Axes.Find(a => a.Position == "bottom").RendererFunction = renderer;
            }
        }
